...
const Categories = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ name: '', description: '', slug: '', isActive: true, isShowHome: false });
      fetchCategories();
    } catch (error) {
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {categories.map((category) => (
              <tr key={category._id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                    category.isShowHome ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
                  }`}>
                ...
                <div className="mb-4">
                  <label className="flex items-center">
                    <input
                      ...
                      onChange={(e) => setFormData({ ...formData, isShowHome: e.target.checked })}
                      className="form-checkbox h-4 w-4 text-blue-600"
                    />
                    <span className="ml-2 text-gray-700">Hiển thị trên trang chủ</span>
                  </label>
                  <p className="text-xs text-gray-500 mt-1">
